...
public class SearchServiceImpl3 {
	 ...
	 */
	public List<SearchHomeVO> getNearHomeAll(SearchHomeVO searchInfo, List<BrtcSignguVO> list) throws XPathExpressionException, IOException, ParserConfigurationException, SAXException{
		...
		for(int i = 0; i<list.size(); i++) {
			...
			for(int h = 0; h<Home.size(); h++) {
				...
				String rnadress = homeData.getRnadres();
				
				SearchHomeVO latlng =  getLatiLongs(homeData.getRnadres());
				if(latlng != null) {
					String homeLat = latlng.getLati();
					String homeLng = latlng.getLongs();
					...
					if(dist<800) {//800미터 안쪽에있는 주택들 리스트에 넣기
						if(!rnadres.equals(homeData.getRnadres())) {
							homeData.setLati(homeLat);
							homeData.setLongs(homeLng);
	 ...
	 * 선택한 주택 상세정보.
	 */
	public List<SearchHomeVO> getHomeDetail(SearchHomeVO searchInfo, List<BrtcSignguVO> list){
		String searchAddress = searchInfo.getRnadres();
		List<SearchHomeVO> homeDetail = searchdao.getHomeDetail(searchAddress);
		System.out.println("homDetail : " +homeDetail.toString());
		return homeDetail;
	}	
