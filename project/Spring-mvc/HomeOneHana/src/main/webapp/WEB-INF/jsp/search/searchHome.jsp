	...
	<main style="display: flex;">
		...
		<aside class="sidebar">
					...
					<div id="homelist" class="list-group list-group-flush border-bottom scrollarea">
						<div class="list-group-item list-group-item-action py-3 lh-tight">
							<div class="d-flex w-100 align-items-center justify-content-between">
								<strong class="mb-1">지금 모집중인 공고를 확인하세요!</strong>
							</div>
							<div class="d-flex w-100 align-items-center justify-content-between">
						...
						<c:forEach items="${ requestScope.notices }" var="noti" varStatus="loop">
							<div class="list-group-item list-group-item-action py-3 lh-tight">
						        <div class="d-flex w-100 align-items-center justify-content-between">
						          <strong id="Strongaddr${loop.index }" class="mb-1"><a href="${noti.value.dtl_url }" >${ noti.key } </a></strong>
						        </div>
						        <div class="col-10 mb-1 small">${noti.value.pan_nm} </div>
						   </div>
						</c:forEach>
					</div>
				</c:if>
				...
				<c:if test="${ not empty requestScope.nearHome }">
					<div id="homelist" class="list-group list-group-flush border-bottom scrollarea">
						<c:forEach items="${ requestScope.nearHome }" var="Home" varStatus="loop">
							<div id="AddrBox${loop.index }" class="list-group-item list-group-item-action py-3 lh-tight">
						        <div class="d-flex w-100 align-items-center justify-content-between">
						          <strong id="Strongaddr${loop.index }" onmouseover="mover(this.id)" onmouseout="mout(this.id)" class="mb-1"><a href="${pageContext.request.contextPath }/search/Detail/${ Home.rnadres }" > ${Home.rnadres}</a></strong>
		...
		<section>
			<!-- 지도 fdb0fd8047707bf8af7800399c2f657d -->
