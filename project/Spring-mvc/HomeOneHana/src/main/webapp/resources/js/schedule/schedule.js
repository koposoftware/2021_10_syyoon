...
const goToday = () => {
  ...
  renderCalender();
	...
	function getLocationClass(brtcName){
		...
		else if(brtcName == '울산광역시') return 'ulsan'
	}
	...
	function noticeButtonClick(notice){
		...
		$('.gyounggi').click(function(){
			...
			for(let i=0; i<notice.length; i++){
				...
				if(notinfo.cnp_cd_nm == '경기도' && notinfo.pan_nt_st_dt == getdate){
					...
					const pnm=encodeURI(notinfo.pan_nm)
	
				
					$('.modal-body  table tbody').append('<tr><td>'+notinfo.ais_tp_cd_nm
					+'</td><td><a href="/HomeOneHana/schedule/detail?pan_nm='+pnm
															...
															+'&dtl_url='+urls
															+'&menu_id='+menuid+'">'+notinfo.pan_nm+'</a></td><td>'+notinfo.clsg_dt+'</td></tr>')
		...
		$('.daejeon').click(function(){
			...
			for(let i=0; i<notice.length; i++){
				let notinfo = notice[i]
				if(notinfo.cnp_cd_nm == '대전광역시' && notinfo.pan_nt_st_dt == getdate){
					const urls = notinfo.dtl_url.split('?')[1].split('&')[0].split('=')[1]
					const menuid = notinfo.dtl_url.split('&')[1].split('=')[1]
					const pnm=encodeURI(notinfo.pan_nm)
		...
		$('.ulsan').click(function(){
			...
			for(let i=0; i<notice.length; i++){
				let notinfo = notice[i]
				if(notinfo.cnp_cd_nm == '울산광역시' && notinfo.pan_nt_st_dt == getdate){
					const urls = notinfo.dtl_url.split('?')[1].split('&')[0].split('=')[1]
					const menuid = notinfo.dtl_url.split('&')[1].split('=')[1]
					const pnm=encodeURI(notinfo.pan_nm)
